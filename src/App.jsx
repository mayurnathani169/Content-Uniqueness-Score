...
const ANALYSIS_PROMPT = (content) => `You are a strict content quality auditor evaluating blog content against Google's E-E-A-T framework (Experience, Expertise, Authoritativeness, Trustworthiness) — the standard Google uses to determine if content is genuinely helpful or just made to manipulate search rankings.
...
Evaluate this content using Google's official E-E-A-T framework and return ONLY a valid JSON object. No markdown, no backticks, no explanation whatsoever:

{
  ...
  "eeat_scores": {
    ...
    "people_first_content": <0-100>
  },
  "verdict_summary": "<2-3 sentence plain English verdict referencing EEAT>",
  "red_flags": ["<specific EEAT red flag 1>", "<specific EEAT red flag 2>", "<specific EEAT red flag 3>"],
  "what_works": ["<specific EEAT strength 1>", "<specific EEAT strength 2>"],
  "hire_verdict": "<HIRE | PROBATION | FIRE>",
  "google_verdict": "<HELPFUL | NEEDS WORK | UNHELPFUL>"
}

E-E-A-T Scoring Guide (based on Google's official quality rater guidelines):

EXPERIENCE (0-100):
- Does the content show first-hand experience? (e.g. "I tested this", "we tried this", real examples)
- Are there specific details only someone with real experience would know?
- Does it go beyond summarizing what others say?
- 0 = zero first-hand experience, pure summary. 100 = rich real-world experience throughout.

EXPERTISE (0-100):
- Does it demonstrate deep subject knowledge beyond surface-level facts?
- Is there insightful analysis that goes beyond the obvious?
- Would a genuine expert in this field find it accurate and substantive?
- Are there any easily-verified factual errors?
- 0 = no expertise, generic filler. 100 = clearly written by a subject matter expert.

AUTHORITATIVENESS (0-100):
- Does the content feel like it comes from a credible, established source?
- Is there author information, credentials, or a byline?
- Does it cite external sources or data to back up claims?
- Would you expect to see this referenced in a reputable publication?
- 0 = anonymous, uncredentialed, no sources. 100 = clearly authoritative with strong credibility signals.

TRUSTWORTHINESS (0-100):
- Is the content transparent about who created it and why?
- Is it free from manipulative, clickbait, or misleading claims?
- Does it feel created to genuinely help people vs. purely to rank on Google?
- Is it well-produced with no sloppy writing, spelling issues, or hasty creation signs?
- 0 = highly manipulative, zero trust signals. 100 = fully transparent and trustworthy.

PEOPLE-FIRST CONTENT (0-100):
- Is this content clearly created for real users, not to game search rankings?
- After reading, would someone feel they learned enough to achieve their goal?
- Does it avoid: mass-produced feel, keyword stuffing, fake freshness, trending topics without real expertise?
- Does it avoid generic filler like "In today's fast-paced world", "It's important to note", "In conclusion"?
- 0 = pure search-engine-first, manipulative content. 100 = genuinely helpful, people-first content.

hire_verdict: HIRE if overall >= 70, PROBATION if 40-69, FIRE if below 40
google_verdict: HELPFUL if overall >= 70, NEEDS WORK if 40-69, UNHELPFUL if below 40

Be brutally honest. Small businesses use this to evaluate if their SEO agency is creating content that Google will actually reward — or content that will hurt their rankings.`;
    ...
    <div style={{ minHeight: "100vh", background: "#0b0f1a", fontFamily: "'DM Sans', sans-serif", color: "#e0e8ff" }}>
      ...
      <div style={{ borderBottom: "1px solid #1a2235", padding: "20px 32px", display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <span style={{ fontSize: "10px", color: "#00e5a0", fontFamily: "'DM Mono', monospace", padding: "3px 8px", border: "1px solid #00e5a033", borderRadius: "4px" }}>EEAT POWERED</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "48px 24px" }}>
        ...
        <div style={{ marginBottom: "48px" }}>
          ...
          <p style={{ color: "#5a6a8a", fontSize: "15px", lineHeight: 1.7, maxWidth: "540px", margin: 0 }}>
            Paste any blog URL and get a score based on Google's official <strong style={{color:"#8899bb"}}>E-E-A-T framework</strong> — Experience, Expertise, Authoritativeness, Trustworthiness. If your content isn't unique, your competitors are getting the traffic you're missing.
          </p>
        </div>
          ...
          <div className="fade-up">
            ...
            <div style={{ background: "#0f1525", border: "1px solid #1a2235", borderRadius: "16px", padding: "32px", marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
                <h3 style={{ margin: 0, fontSize: "11px", color: "#5a6a8a", fontFamily: "'DM Mono', monospace", letterSpacing: "0.12em" }}>GOOGLE E-E-A-T BREAKDOWN</h3>
                {result.google_verdict && (
                  <div style={{ padding: "4px 12px", borderRadius: "4px", background: result.google_verdict === "HELPFUL" ? "#00e5a010" : result.google_verdict === "NEEDS WORK" ? "#f5c54210" : "#ff4d6d10", border: `1px solid ${result.google_verdict === "HELPFUL" ? "#00e5a033" : result.google_verdict === "NEEDS WORK" ? "#f5c54233" : "#ff4d6d33"}` }}>
                    <span style={{ fontSize: "10px", fontFamily: "'DM Mono', monospace", fontWeight: "700", color: result.google_verdict === "HELPFUL" ? "#00e5a0" : result.google_verdict === "NEEDS WORK" ? "#f5c542" : "#ff4d6d", letterSpacing: "0.1em" }}>
                      GOOGLE: {result.google_verdict}
                    </span>
                  </div>
                )}
              </div>
              <SubScoreBar label="⚡ EXPERIENCE — First-hand knowledge & real examples" score={result.eeat_scores.experience} delay={100} />
              <SubScoreBar label="🎓 EXPERTISE — Subject depth & accurate insights" score={result.eeat_scores.expertise} delay={200} />
              <SubScoreBar label="🏆 AUTHORITATIVENESS — Credibility & source signals" score={result.eeat_scores.authoritativeness} delay={300} />
              <SubScoreBar label="🔒 TRUSTWORTHINESS — Transparency & honesty" score={result.eeat_scores.trustworthiness} delay={400} />
              <SubScoreBar label="👥 PEOPLE-FIRST — Written for users, not search engines" score={result.eeat_scores.people_first_content} delay={500} />
            </div>
